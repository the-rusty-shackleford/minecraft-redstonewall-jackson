 ...
 * support (where a wire steps down to); and, when the block in front is a wire of another
 * plane, which of this wire's sides it rests toward. The adapter builds it from the level; the
 * rules never see the level. Immutable.
 *
 * <p>RI: every planar direction has its three cells; {@code bestOtherSignal} is 0..15;
 * {@code frontRestsOn} is present only when {@code above} is a wire.
 *
 ...
 * @param frontRestsOn    the side of this wire that the wire in front rests toward, when the wire in front
 *                        lies in the plane at right angles through that side (the inside corner); empty
 *                        when nothing is in front, or a wire there rests toward this wire's up or support
 */
public record View(Cell above, Map<Planar, Cell> side, Map<Planar, Cell> sideUp, Map<Planar, Cell> sideDown, int bestOtherSignal,
                   Optional<Planar> frontRestsOn) {

    public View {
        Objects.requireNonNull(above, "above");
        Objects.requireNonNull(frontRestsOn, "frontRestsOn");
        ...
        if (frontRestsOn.isPresent() && !above.wire()) {
            throw new IllegalArgumentException("only a wire in front rests on a side");
        }
    }
    ...
    /** effects: returns a view built from three functions of the planar direction, nothing resting in front */
    public static View of(Cell above, int bestOtherSignal, Function<Planar, Cell> side, Function<Planar, Cell> sideUp,
        ...
        return new View(above, s, u, d, bestOtherSignal, Optional.empty());
    }
    ...
    public View withSide(Planar p, Cell cell) {
        ...
        s.put(p, cell);
        return new View(above, s, sideUp, sideDown, bestOtherSignal, frontRestsOn);
    }
    ...
    public View withSideUp(Planar p, Cell cell) {
        ...
        u.put(p, cell);
        return new View(above, side, u, sideDown, bestOtherSignal, frontRestsOn);
    }
    ...
    public View withSideDown(Planar p, Cell cell) {
        ...
        d.put(p, cell);
        return new View(above, side, sideUp, d, bestOtherSignal, frontRestsOn);
    }

    /** effects: returns this view with {@code cell} in front of the wire, resting on no side */
    public View withAbove(Cell cell) {
        return new View(cell, side, sideUp, sideDown, bestOtherSignal, Optional.empty());
    }

    /** effects: returns this view with a wire of {@code power} in front, resting toward side {@code p}: the inside corner */
    public View withFrontRestingOn(Planar p, int power) {
        return new View(Cell.wire(power), side, sideUp, sideDown, bestOtherSignal, Optional.of(p));
    }

    /** effects: returns this view with {@code signal} as the strongest non-wire neighbour signal */
    public View withSignal(int signal) {
        return new View(above, side, sideUp, sideDown, signal, frontRestsOn);
    }
}
